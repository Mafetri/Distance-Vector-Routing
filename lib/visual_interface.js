...
visual_interface.update_routing_table = (node, table) => {
	const routing_table = node_tables.getElementById(node + "_routing_table");
	if (routing_table) {
		const tbody = routing_table.tBodies[0];
		Object.keys(table).forEach((col) => {
			const row = tbody.rows.namedItem(col);
			const td = row.cells[1];
			const value = table[col];
			td.textContent = value;
		});
	}
};
