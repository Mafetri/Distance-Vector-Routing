...
visual_interface.edges_forms = (edges, nodes) => {
	...
	edges.forEach((edge) => {
		...
		// Create options without the edge's nodes
		nodes.filter((node) => !edge.nodes.includes(node)).forEach((node) => {
			nodes_options += `<option value="${node}">${node}</option>`;
		});

		edges_forms += `
			<form class="change_edge_form">
				<p>${edge.nodes[0]}</p>
				<input type="text" name="weight" placeholder="Weight" value="${edge.cost}"/>
				<p>${edge.nodes[1]}</p>
