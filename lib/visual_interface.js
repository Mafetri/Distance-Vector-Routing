...
// Updates the edge given in the graph
...
// Removes and regraphs the network
...
// Highlight the edge used in the graph
...
// Adds a node to the tables
visual_interface.add_node = (node, vectors_table, routing_table) => {
	const tablesDiv = document.getElementById("tables");
	graph_node_div(node, tablesDiv);
	graph_table(node, vectors_table, document.getElementById(node + "_div_tables"));
...
// Updates the vectors table of a node
...
visual_interface.edges_forms = (edges, nodes) => {
	...
	edges.forEach((edge) => {
		edges_forms += `
			<form class="form change_edge_form">
				<p>${edge.nodes[0]}</p>
...
visual_interface.node_form = (nodes) => {
	...
	nodes_form = `
		<form class = "form" id = "delete_node_form">
			<select name="node_name">
			${options}
			</select>
			<button type="submit" value="delete" class = "node_form_delete">Delete</button>
		</form>`;

	document.getElementById("change_node_forms_div").innerHTML = nodes_form;
};

// ==================== Local Methods ====================
function graph_node_div(node, tablesDiv) {
